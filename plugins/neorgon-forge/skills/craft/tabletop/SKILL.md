---
name: tabletop
description: "Use when building or changing a tabletop game that also runs as software: a card game, a boss rush, a deck of prompts, a print-and-play, a facilitator's exercise deck. Triggers on: 'add a card', 'balance this game', 'the game is too hard', 'build a game for', 'make a print-and-play', 'write the rulebook', 'simulate the game', 'is this card worth playing', 'add a mechanic'. Covers the order the pieces must be built in (rulebook, data, engine, screen), the single-source rule that keeps a printed component count from drifting from the data, how to measure a balance claim instead of arguing it, and the specific ways a game-as-software breaks that ordinary code review does not catch: a card the simulator cannot see, a rule the printer drops, a new option that is strictly worse than a free existing one. Not for pure video-game loops (no printed artifact), not for the visual design of a card face alone (use impeccable), and not for writing the rulebook's prose once the rules are settled (use penname or voicecheck)."
