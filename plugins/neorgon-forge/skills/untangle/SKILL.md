---
name: untangle
description: "Use when a problem resists a plan — the cause is unclear, the evidence contradicts itself, several theories fit, a fix keeps not working, or a design has three defensible answers and choosing wrong is expensive. Classifies the difficulty first (unknown cause / undecided design / too large to hold), then runs the branch for it: competing hypotheses each with the observation that would kill it, options scored against stated criteria, or a coverage map before any edit. Triggers on: 'I have no idea why', 'this makes no sense', 'we tried everything', 'it works locally but not in prod', 'intermittent', 'flaky', 'which approach should we take', 'I keep going in circles', 'this is a hard one'. Not for work whose shape is already known (use task) and not for reviewing finished code."
