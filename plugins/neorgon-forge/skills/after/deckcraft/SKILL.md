---
name: deckcraft
description: "Use when a slide deck for slides-site (Presentation Sage) has to be written from an idea, or when a deck that already exists needs fixing or critiquing. Triggers on: 'write me a deck about', 'turn this into slides', 'review my deck', 'these slides are boring', 'my headings are weak', 'rewrite these headings', 'make this deck land', 'prep me for the board', 'tighten this before the talk', 'use deckcraft'. Fixes the failure a density audit cannot see: a deck that validates cleanly and still says nothing, because every heading names a topic instead of stating a claim. Covers heading rewrites, argument order for the room, slide-type choice, speaker notes, and the export traps that silently drop content. Not for a deck about a codebase, either what changed in it or what it is (use debrief), not for a blog post (use writeup), not for marketing page copy (use voicecheck)."
