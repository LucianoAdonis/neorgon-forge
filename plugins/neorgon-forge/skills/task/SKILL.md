---
name: task
description: "Use when the user brings a problem to solve rather than an edit to make — a feature, a refactor, a migration, a bug with an unclear cause, or anything phrased as a goal instead of a diff. Scopes the work before writing code, splits large work into workstreams that can be delegated to subagents and verified on return, and keeps a brief on disk so the reasoning survives the session. Triggers on: 'implement X', 'add support for Y', 'refactor Z', 'figure out why W is broken', 'migrate A to B', 'I want to build', 'take on this whole thing', 'this is a big one'. Not for a one-line change you already know how to make, and not for reviewing existing code (use code-review)."
