...
## Step 7: check the model against the language, where there is one

A dependency model knows what the code calls things. It does not know what the *project* calls
them, and the gap between the two is where a reader gets lost: the glossary says **decision
ticket** and the module is `items.js`.

When `.forge/context.md` exists (written by `grill`), read it and report the drift, both
directions:

- **A glossary term with no home in the model.** Either the concept lives somewhere the extractor
  cannot see, or it is vocabulary the code never adopted. Both are worth a line in the docs.
- **A model node whose name contradicts the glossary's `_Avoid_` list.** The code is using a word
...
Where no `context.md` exists, skip this step in silence rather than inventing a glossary. A
generated glossary is a second set of names with nothing behind it.
