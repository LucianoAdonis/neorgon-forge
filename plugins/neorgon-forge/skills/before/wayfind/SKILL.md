---
name: wayfind
description: "Use when the same 'where does this go' question keeps being answered from scratch, or when a ticket has to be resolved to actual files before anything can be changed. Triggers on: 'I don't know this codebase', 'map this repo before we start', 'where do changes of this kind go', 'what are the conventions here', 'I keep guessing wrong about where things live', 'which files does this ticket touch', 'do two of these repos do the same job'. Builds a durable map in .forge/map.md: areas in the user's words, path rules carrying how each was established, and the exceptions practice turns up, then resolves a ticket to candidate files and the rules over them. A multi-repo workspace gets one map per child plus an index flagging concerns two repos share. Not for one lookup you could grep, not for diagnosing a bug (use untangle), not for executing the change once the location is known (use task)."
