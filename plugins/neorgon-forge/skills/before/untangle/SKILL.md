---
name: untangle
description: "Use when a problem resists a plan and starting work would be guessing: the cause is unknown, the evidence contradicts itself, or several designs are each defensible. Triggers on: 'figure out why X is broken', 'why does this fail', 'find the root cause', 'I have no idea why', 'we tried everything', 'it works locally but not in prod', 'intermittent', 'flaky', 'the fix did not hold', 'which approach should we take', 'weigh the options', 'trade-offs'. Classifies the difficulty first (unknown cause, undecided design, too large to hold), then runs the branch for it: competing hypotheses each carrying the observation that would kill it, options scored against criteria written before the options, or a coverage map before any edit. Not for a bug whose cause is already known (use task to fix it), not for stress-testing a plan already chosen (use grill), and not for reviewing finished code."
