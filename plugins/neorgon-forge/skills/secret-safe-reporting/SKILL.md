...
description: "Use when building anything that reads sensitive data and produces output others will see. A secrets/credentials scanner, a compliance dashboard, a PII audit, an error reporter that touches user content, an LLM trace viewer, tests written against real observed data. Triggers on: 'report on secrets', 'scan for credentials', 'compliance dashboard for sensitive data', 'don't leak PII into the report', 'audit artifact with sensitive data', 'write tests for the classifier'. Covers the boundary architecture that makes leaks structurally impossible, classifier ordering, synthetic fixtures, canary enforcement, and the pre-publish history sweep. Not for rotating an already-leaked credential (that is incident response, and rotation comes first), and not a replacement for gitleaks/trufflehog in CI. It is for designing the pipeline those tools then double-check."
